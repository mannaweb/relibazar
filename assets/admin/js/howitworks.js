function saveData(){
  ...
  $.ajax({
    ...
    url: BASE_URL +'admin/how-it-works-save-data',
    ...
    success:function(result){
      ...
      if(data.status == 1){
        ...
        toastr.success(data.msg, 'Saved', {timeOut: 1000,progressBar:true,onHidden:function() { window.location.href = BASE_URL+'admin/how-it-works'; }   });
...
function searchFilter(page_num,loadingType) {
  ...
  $.ajax({
    ...
    url: BASE_URL+'admin/how-it-works-search-data/'+page_num,
    ...
    success: function (html) {
      ...
      $('#howitworkList').html(data.html);

      $('#paginationDiv').html(data.pagination);

      if(loadingType != 1){

        $('html, body').animate({scrollTop:$('#howitworkList').offset().top - 200 }, 500);
...
function deleteHowItWork(id,status){
...
function deleteConfirm(){
  ...
  $.ajax({
    ...
    url: BASE_URL+'admin/delete-how-it-works',
...
function statusChange(id,status){
  ...
  $.ajax({
    ...
    url: BASE_URL+'admin/change-status-how-it-works',
...
function sortableEnable(){
  ...
  $( "#howitworkList" ).sortable({
    ...
    update  : function(event, ui){
      ...
      $('#howitworkList tr').each(function(index){

        $(this).find(".ordering_val").html(parseInt(index)+parseInt(start));

        $(this).find(".ordering").val(parseInt(index)+parseInt(start));
...
function saveSorting(){
    ...
    $('tbody tr').each(function(index){
        ...
        ids.push($(this).find(".product").val());
    ...
    $.ajax({
            ...
            url: BASE_URL +'admin/save-how-it-works',
...
$(document).ready(function(){
  ...
  if( param[param.length - 1] && param[param.length - 1] == 'how-it-works' ){
...
$('#startEnd').on('cancel.daterangepicker', function(ev, picker) {
  ...
  $(".cancelButton").click(function(){
    ...
    window.location.href=BASE_URL+"admin/how-it-works";
