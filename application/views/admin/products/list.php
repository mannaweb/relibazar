	...
	<div class="card-header py-3 forms">
		<div class="row form-row mb-2">
			<div class="col-sm-6 d-flex">
				<h6 class="title my-auto">Products list</h6>
			</div>
			<div class="col-sm-6 text-right">
				<?php if(getUserDetails($this->session->userdata('user_id'),'access_management') && in_array('add-product', getUserDetails($this->session->userdata('user_id'),'access_management'))){?>
				<a href="<?php echo base_url()?>admin/add-product" class="btn btn-sm btn-success" data-tooltip="Add"><i class="fa fa-plus"></i></a>
				<?php } ?>
				...
				<a href="javascript:void(0)" class="btn btn-sm btn-danger" onclick="deleteProduct('all')" data-tooltip="Delete"><i class="fa fa-trash"></i></a>
				<?php } ?>
			...
			<div class="col-sm-3 col-md-3 col-lg-4 col-xl-4">
				<div class="form-group">
					<select class="form-control select2" id="category" onchange="searchFilter()">
						...
						<option value="<?php echo $value->id; ?>" <?php if($value->id == $catGet){ echo 'selected';}?>><?php echo $value->name; ?></option>
            <?php } } ?>   						
					</select>
				</div>
			</div>
