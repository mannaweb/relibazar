...
if(!function_exists('getDecode')){ 
    function getDecode($product_id=''){ 
       $ci = @get_instance();
        ...
        $qu = $ci->db->get();
        $row = $qu->row();
        //print_r($row->selling_price);die;
        return json_decode($row->selling_price,true);
    }
...
if(!function_exists('getBanner')){ 
 function getBanner($array=array()){    
    ...
    if(isset($array['type']) && $array['type'] == 'home'){
        $ci->db->from('banners');
        $ci->db->select('type,type_id,title,description,image,link');
...
if(!function_exists('getPopularCat')){ 
   function getPopularCat($array=array()){    
    ...
    $ci->db->select('categories.id,categories.name,categories.slug,categories.short_description,categories.image');
...
if(!function_exists('getFeaturedProduct')){ 
   function getFeaturedProduct($array=array()){    
    $ci = @get_instance();
    $ci->load->database();
    $ci->db->select('products.id,products.category_ids,products.name,products.slug,products.image,products.selling_price,products.tax,products.product_type,categories.name as category_name');
    $ci->db->from('products');
    $ci->db->join('categories','categories.id = products.category_ids','left');
...
if(!function_exists('settingsFileExists')){ 
   function settingsFileExists($file_url,$file){    
    if($file && file_exists($file_url)){
        return base_url().$file_url;
    } else {
        return base_url().'assets/admin/img/no-image.jpg';
...
if(!function_exists('getBreadcumbName')){
   function getBreadcumbName(){
        $ci = @get_instance();
        if($ci->uri->segment(2) == ''){
          $segemnt = $ci->uri->segment(1); 
       }else{
          $segemnt = $ci->uri->segment(2);
       }
        $result = ucwords(str_replace(['_','-'], ' ', $segemnt));
        return $result;
    } 
}
...
if(!function_exists('getProductIdToSlug')){
    function getProductIdToSlug($product_id=''){
        $ci = @get_instance();
        $ci->load->database();
        $ci->db->from('products');
        $ci->db->where('id',$product_id);
...
if(!function_exists('getAllProductByCategory')){
    function getAllProductByCategory($category_id=''){
        $ci = @get_instance();
        $ci->load->database();
        $ci->db->from('products');
        $ci->db->where("FIND_IN_SET(".$category_id.",category_ids) !=", 0);
        $query = $ci->db->get();
...
//*********************** Cart System ********************//
    if(!function_exists('getAllCartData')){
        function getAllCartData($session_id=''){
            $ci = @get_instance();
            $ci->load->database();
            $ci->db->from('cart');
...
//*********************** Bulk sms getway ********************//
   if(!function_exists('getOtpBySms')){
    function getOtpBySms($phone='',$message='',$otp=''){
        $ci = @get_instance();
        $ci->load->database();
        ...
        $apiUrl = "http://103.247.98.91/API/SendMsg.aspx?uname=".$username."&pass=".$password."&send=".$send."&dest=".urlencode($phone)."&msg=".rawurlencode($message);
        $curl = curl_init();
...
if(!function_exists('gmTokg')){
    function gmTokg($price='',$parm=''){
      if($parm == 'gm'){
        if($price >= 1000){
           $price  = number_format($price / 1000, 2);
           $parm = 'Kg';
           return $price.$parm;
        }else{
          return $price.$parm;
