...
class Categories_model extends CI_Model {
	...
	public function getList($params = array(),$limit = array()){
		...
		if(array_key_exists("start",$limit) && array_key_exists("limit",$limit)){
		 ...
		 $this->db->where('categories.type',1);
	...
	public function saveData($data=array()){
		...
		if($data['id']){
			...
			$this->db->insert('categories',array('image'=>$data['image'],'name'=>$data['name'],'slug'=>$data['slug'],'short_description'=>$data['short_description'],'long_description'=>$data['long_description'],'created_by'=>$data['created_by']));
		}
