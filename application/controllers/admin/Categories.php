...
class Categories extends CI_Controller {
	...
	public function changeFeaturedCategory(){
		$return = $this->categories_model->changeFeaturedCategory($this->input->post());
		echo json_encode($return);
	}
    ...
    public function saveOrdering(){
		$return = $this->categories_model->saveOrdering($this->input->post());
		echo json_encode($return);
	}

	public function getSub(){
		$id = $this->input->post('id');
		$data['all_sub_cat'] = $this->db->get_where('categories', array('root_id'=>$id,'status'=>1,'type'=>2))->result();
		 $return['html'] = $this->load->view('admin/ajax/select',$data,true);
