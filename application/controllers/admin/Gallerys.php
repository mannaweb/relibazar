...
class Gallerys extends CI_Controller {
	...
	public function manageGallery($alias=''){
		...
		$data['alias'] = $alias;
		$post_id = getProductDetailsByAlias($alias,'id');
	...
	public function GateData(){
		...
		$value = $this->db->get_where('product_galleries',array('product_id'=>$product_id))->result_array();
	...
	public function doUpload($FILES,$file){
		if($FILES['file']['name']){
			...
			$config['upload_path']          = 'uploads/products/gallery';

			if(!is_dir($config['upload_path'])){
				mkdir($config['upload_path'],0777,TRUE);
			}
			if(!is_dir('uploads/products/gallery/150x150')){
				mkdir('uploads/products/gallery/150x150',0777,TRUE);
			}
			...
			if ( ! $this->upload->do_upload('file')){
				...
				@unlink('uploads/products/gallery/150x150/'.$file);

				
				
					$this->load->library('image_lib');
				    $config['image_library'] = 'GD2';
				    $config['source_image'] = 'uploads/products/gallery/'.$this->upload->data()['file_name'];
				    $config['maintain_ratio'] = TRUE;
				    $config['width']     = '150';
				    $config['height']   = '150';
				    $config['new_image']   = 'uploads/products/gallery/150x150/'.$this->upload->data()['file_name'];
